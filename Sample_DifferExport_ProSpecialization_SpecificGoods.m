...
% Export restriction: 50% for MANR-C；MANR-C, MANR-B
% T = 82;
% close all
clear
clc
...
for sc =1:size(Scenarios,1)
    for sf = 1:size(Flood_set_int,1)
        ...
        spr = Sprod(:,2);
        for i = 1:size(spi,1)
            I_sum(spi(i),spi(i)+(spr(i)-1)*S) = 0;
            I_sum(S+i,spi(i)+(spr(i)-1)*S) = 1;  % specialized product
        end
        
        si = Scenarios{sc}(:,1); % Export restrictions on specific goods
        sr = Scenarios{sc}(:,2);
...
%% End.
save FigureData_differexportsp.mat R T Scenarios Flood_set_int VA_CT VA_OT VA_AT VA_DT
