...
% T = 82
% on inventory size: 2 4 6 8 10 12
% close all
clear
clc
%% Main loop
...
% Inventory size
StockPs = [2 4 6 8 10 12];  % weeks of production sustained by stock
...
VAL = zeros(size(StockPs,2),S*R);
VAL_T = zeros(size(StockPs,2),T);
VA_AT = zeros(size(StockPs,2),T);
VA_BT = zeros(size(StockPs,2),T);
VA_CT = zeros(size(StockPs,2),T);
VA_DT = zeros(size(StockPs,2),T);

for ss = 1:size(StockPs,2)
    ...
    %%%% stock & order
    StockP = StockPs(ss);  % weeks of production sustained by stock
...
% legend
lgd = legend('Pre-disaster','InvSize = 2 weeks','InvSize = 4 weeks','InvSize = 6 weeks','InvSize = 8 weeks','InvSize = 10 weeks','InvSize = 12 weeks');
